...
import type { Content, PhrasingContent, Root } from 'mdast-util-from-markdown/lib'
...
const leafDirectiveToJSX = (name: string, attributes: Record<string, string | undefined | null>, children: PhrasingContent[]) => {
  switch (name) {
    case 'try-it-controls':
      return <TryItControls utilities={Object.values(attributes).filter(val => typeof val === 'string') as string[]} />
    case 'try-it-preview':
      setHtml(attributes.html ?? '')
      return <ShadowDomUnoCSS
        html={html()}
    ...
    default:
      return <Error>Unsupported directive: {name}</Error>
  }
}
